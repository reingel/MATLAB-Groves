...
from Kinematics_ECI import Kinematics_ECI
from Nav_equations_ECI import Nav_equations_ECI
...
def Inertial_navigation_ECI(in_profile, no_epochs, initialization_errors, IMU_errors):
    """
    Simulates inertial navigation using ECI navigation equations and kinematic model
    ...
    old_true_r_ib_i, old_true_v_ib_i, old_true_C_b_i = ECEF_to_ECI(
        old_time, true_r_eb_e, true_v_eb_e, true_C_b_e)
    ...
    old_est_r_ib_i, old_est_v_ib_i, old_est_C_b_i = ECEF_to_ECI(
        old_time, old_est_r_eb_e, old_est_v_eb_e, old_est_C_b_e)
    ...
    for epoch in tqdm(range(1, no_epochs), desc="Processing", ncols=80):
        ...
        true_r_eb_e, true_v_eb_e, true_C_b_e = NED_to_ECEF(
            true_L_b, true_lambda_b, true_h_b, true_v_eb_n, true_C_b_n)
        true_r_ib_i, true_v_ib_i, true_C_b_i = ECEF_to_ECI(
            time, true_r_eb_e, true_v_eb_e, true_C_b_e)

        # Time interval
        tor_i = time - old_time

        # Calculate specific force and angular rate
        true_f_ib_b, true_omega_ib_b = Kinematics_ECI(
            tor_i, true_C_b_i, old_true_C_b_i, true_v_ib_i, old_true_v_ib_i, old_true_r_ib_i)
        ...
        est_r_ib_i, est_v_ib_i, est_C_b_i = Nav_equations_ECI(
            tor_i, old_est_r_ib_i, old_est_v_ib_i, old_est_C_b_i, meas_f_ib_b, meas_omega_ib_b)
